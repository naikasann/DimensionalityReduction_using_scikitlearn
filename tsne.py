...
def TSNE_2D(X_data, Y_data, output_directory):
    ...
    for cnt, (x, y) in enumerate(zip(plot_xlist, plot_ylist)):
        plt.scatter(x, y, cmap=cm.tab10, marker=markers[cnt])
    ...
    # save figure.
    fig.savefig(output_directory + "TSNE_result_2D.png")
...
def PCA_3D(X_data, Y_data, output_directory):
    ...
    anim = animation.FuncAnimation(gif, animation_for_3D,
                                    ...
                                    blit=True)
    # Gif image save.
    anim.save(output_directory + "TSNE_result_3D.gif", writer="imagemagick")
...
def main():
    ...
    if args.arg1 == "3D":
        ...
        TSNE_2D(X_data, Y_data, config["output_directory"])
    else:
